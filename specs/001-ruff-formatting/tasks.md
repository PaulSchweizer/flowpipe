...
- [ ] T001 Update `pyproject.toml` and `poetry.lock` to remove `black`/`isort` dev dependencies and add `ruff` under `[tool.poetry.group.dev.dependencies]`.
...
- [ ] T002 Define the `[tool.ruff]` configuration (line length, target version, minimal rule toggles) and delete `[tool.black]`/`[tool.isort]` sections inside `pyproject.toml`.
...
- [ ] T003 [US1] Replace the Black/isort repos in `.pre-commit-config.yaml` with the official `astral-sh/ruff-pre-commit` hooks (`ruff` with `--fix` and `ruff-format`) so only Ruff enforces style before commits.
- [ ] T004 [P] [US1] Update `.vscode/settings.json` to configure VS Code’s default formatter/linting to use the Ruff extension (and remove the `python.formatting.provider = "black"` binding).
- [ ] T005 [P] [US1] Refresh `.flake8` comments/ignores to reference Ruff’s formatting expectations (e.g., keep `E203/W503` rationale) so no references to Black/isort remain in lint configuration.
...
- [ ] T006 [P] [US2] Update `README.md` badges/instructions to highlight Ruff (swap the Black badge, mention Ruff-driven formatting commands).
- [ ] T007 [P] [US2] Rewrite the formatting guidance in `contributing.md` to describe installing Ruff via pre-commit, running `pre-commit run --all-files`, and removing all Black/isort references.
- [ ] T008 [P] [US2] Author `docs/ruff-formatting.md` (and link it from `docs/index.rst`) detailing installation, troubleshooting stale hooks, and explaining how other tools can inspect graph data using Ruff-formatted code.
...
- [ ] T009 [US3] Add `.github/workflows/pre-commit.yml` that leverages `pre-commit/action@v3` to run the Ruff hooks on every push and pull request.
- [ ] T010 [P] [US3] Insert a `poetry run pre-commit run --all-files --hook-stage manual ruff ruff-format` step near the start of `.github/workflows/pytest.yml` so test pipelines verify Ruff compliance using the existing tooling stack.
...
- [ ] T011 Create `docs/release-notes.md` (and reference it from `README.md` or docs navigation) summarizing the Ruff migration and providing upgrade guidance for downstream consumers.
- [ ] T012 [P] Update `.github/ISSUE_TEMPLATE/bug_report.md` and `.github/ISSUE_TEMPLATE/feature_request.md` to remind contributors to run `pre-commit run --all-files` (Ruff) before submitting changes or reproduction cases.
