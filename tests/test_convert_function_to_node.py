...
def test_passing_metadata_updates_exisiting_metadata(clear_default_graph):
    ...
    node = function(
        metadata={"arg_1": "new_value", "arg3": "new_value"}, graph=None
    )
...
def test_default_args_are_assigned_to_input_plugs(clear_default_graph):
    ...
    assert node.inputs["arg_2"].value == "test_1"
    assert node.inputs["arg_3"].value == "test_2"
...
def test_node_reserved_names():
    with pytest.raises(ValueError):
        ...
        @Node()
        def function(
            func, name, identifier, inputs, outputs, metadata, omit, graph
        ):
            pass
